...
class CartPayment extends StatefulWidget {
  ...
  const CartPayment(
      ...
      required this.ListOfCartItemId1});
...
class _CartPaymentState extends State<CartPayment> {
  ...
  Widget build(BuildContext context) {
    ...
    double discount = widget.discountAmount;

    double baseTotalAmount = widget.totalprice;
    late String paymentMethod;
    double coupanDiscount = 0;
    if (baseTotalAmount < slabAmount1) {
      setState(() {
        totalAmount =
            (widget.totalprice + deliveryCharge) - (coupanDiscount);
        deliverycharge1 = deliverycharge2.toString();
      });
    }
    if (baseTotalAmount > slabAmount1) {
      setState(() {
        totalAmount = widget.totalprice - (coupanDiscount);
              ...
              BlocListener<ApplyCoupansBloc, ApplyCoupansState>(
                listener: (context, state) {
                  ...
                  if (state is ApplyCoupansblocLoaded) {
                    ...
                    coupanDiscount =
                        double.parse(coupans.first.discountAmount.toString());
                    setState(() {
                      isVisible = false;
                    });
                    if (coupans.isEmpty) {
                ...
                child: Row(
                  children: [
                    ...
                    BlocListener<PlaceOrderBloc, PlaceOrderState>(
                      listener: (context, state) {
                        if (state is PlaceOrderblocLoading) {
                          print('loading');
                        }
                        if (state is PlaceOrderblocLoaded) {
                          deliveryTypeId = '';
                          print('loaded');
                        ...
                        if (state is PlaceOrderblocError) {
                        ...
                        onPressed: () {
                          ...
                          if (deliveryTypeId != '' &&
                            ...
                            BlocProvider.of<PlaceOrderBloc>(context).add(
                                FetchPlaceOrder(
                                    deliveryId: widget.addressId,
                                    paymentMethod: paymentMethod,
                                    ListOfCartItemId1: widget.ListOfCartItemId1,
                                    coupan: coupan.text, deiverytypeid:deliveryTypeId ));
                          }
                        },
                        child: Container(
                          width: mwidth * 0.3,
